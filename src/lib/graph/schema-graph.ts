...
export function reorderNode(
  ...
  // Get all child edges - find them by matching targetId with children
  const allChildEdges: Array<{ edge: GraphEdge; targetId: string; currentOrder: number }> = [];
  
  // Build a map of targetId -> edge for quick lookup
  const edgeMap = new Map<string, GraphEdge>();
  newGraph.edges.forEach((edge) => {
    if (edge.sourceId === parentId && edge.type === 'child') {
      edgeMap.set(edge.targetId, edge);
    }
  });
  
  // Build ordered list based on current children order
 ...
 */
export function getDefinitions(graph: SchemaGraph): Map<string, SchemaNode> {
